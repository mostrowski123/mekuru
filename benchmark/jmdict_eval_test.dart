...
const List<EvalQuery> _battery = [
  ...
  EvalQuery(
    ...
    'ship',
    EvalCategory.newFeature,
    expect: [Expect('船', maxRank: 3)],
    note: 'partial romaji し (trailing p) must not flood the exact tier',
  ),
  EvalQuery(
    'new.en.bus',
    'bus',
    EvalCategory.newFeature,
    expect: [Expect('バス', maxRank: 3)],
    note: 'partial romaji ぶ (trailing s)',
  ),
  EvalQuery(
    'new.en.dog',
    'dog',
    EvalCategory.newFeature,
    expect: [Expect('犬', maxRank: 3)],
    note: 'partial romaji ど (trailing g)',
  ),
  ...
  EvalQuery(
    ...
    'run',
    EvalCategory.newFeature,
    expect: [Expect('走る', maxRank: 2)],
    note: 'headline gloss beats やる sense 6 "to run (a business)"',
  ),
  EvalQuery(
    'new.en.play',
    'play',
    EvalCategory.newFeature,
    expect: [Expect('遊ぶ', maxRank: 3)],
    note: 'headline gloss beats 成る sense 8',
  ),
  EvalQuery(
    'new.en.drink',
    'drink',
    EvalCategory.newFeature,
    expect: [Expect('飲む', maxRank: 2)],
  ),
  EvalQuery(
    'new.en.work',
    'work',
    EvalCategory.newFeature,
    expect: [Expect('仕事', maxRank: 2)],
    note: 'headline gloss beats 事 sense 4',
  ),
  EvalQuery(
    'new.en.love',
    'love',
    EvalCategory.newFeature,
    expect: [Expect('愛', maxRank: 2)],
    note: 'headline gloss beats 思う sense 7',
  ),
  EvalQuery(
    'new.en.light',
    'light',
    EvalCategory.newFeature,
    expect: [Expect('光', maxRank: 2)],
    note: 'headline gloss beats 簡単 sense 2',
  ),
  EvalQuery(
    'new.en.dream',
    'dream',
    EvalCategory.newFeature,
    expect: [Expect('夢', maxRank: 2)],
  ),
  EvalQuery(
    'new.en.write',
    'write',
    EvalCategory.newFeature,
    expect: [Expect('書く', maxRank: 2)],
  ),
  EvalQuery(
    'new.en.cook',
    'cook',
    EvalCategory.newFeature,
    expect: [Expect('料理人', maxRank: 3)],
    note: 'headline gloss beats 誤魔化す "to cook (the books)"',
  ),
  EvalQuery(
    'new.en.food',
    'food',
    EvalCategory.newFeature,
    expect: [Expect('食べ物', maxRank: 2)],
    note: 'headline gloss beats 料理 4th gloss',
  ),
  EvalQuery(
    'new.en.water',
    'water',
    EvalCategory.newFeature,
    expect: [Expect('水', maxRank: 1)],
    note: 'partial romaji わて must not deinflect into 割る',
  ),
  EvalQuery(
    'new.en.walk',
    'walk',
    EvalCategory.newFeature,
    expect: [Expect('歩く', maxRank: 2)],
    note: 'control: verb stays first (Jisho shows 散歩)',
  ),
  EvalQuery(
    'new.en.fish',
    'fish',
    EvalCategory.newFeature,
    expect: [Expect('魚', maxRank: 2)],
    note: 'control',
  ),
  EvalQuery(
    'new.en.cut',
    'cut',
    EvalCategory.newFeature,
    expect: [Expect('切る', maxRank: 2)],
    note: 'control: verb stays first (Jisho shows 削減)',
  ),
