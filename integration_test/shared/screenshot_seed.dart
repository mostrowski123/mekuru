// Content for the App Store screenshots: a small library of public-domain
// Japanese books (Natsume Soseki, Dazai Osamu, Miyazawa Kenji) with generated
// covers, and a few dictionary entries written here. Used by the scripted
// tour (app_store_screenshots_test.dart) and by the normal-app entry point
// (app_store_screenshot_app.dart).
import 'dart:convert';
import 'dart:io';
...
/// Seeds [db] with the dictionary and imports the books from [workDir].
/// [offUiThread] wraps work that needs real async (the test binding's
/// `tester.runAsync`); the normal app passes nothing.
Future<void> seedScreenshotLibrary(
  AppDatabase db,
  Directory workDir, {
  Future<T?> Function<T>(Future<T> Function() body)? offUiThread,
}) async {
  await db
  ...
  for (final (i, (title, author, color))
      in screenshotBooks.indexed.toList().reversed) {
    final cover = offUiThread == null
        ? await _cover(title, author, color)
        : await offUiThread(() => _cover(title, author, color));
    final path = await _epub(workDir, i, title, author, cover!);
